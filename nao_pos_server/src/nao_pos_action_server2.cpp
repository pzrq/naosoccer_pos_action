...
#include "nao_pos_server/nao_pos_action_server2.hpp"
...
NaoPosActionServer2::NaoPosActionServer2(const rclcpp::NodeOptions& options)
  : rclcpp::Node{ "nao_pos_action_server2_node", options }
{
  pub_joint_positions_ = create_publisher<nao_lola_command_msgs::msg::JointPositions>("/effectors/joint_positions", 1);
  pub_joint_stiffnesses_ =
      create_publisher<nao_lola_command_msgs::msg::JointStiffnesses>("/effectors/joint_stiffnesses", 1);
  pos_in_action_ = false;

  sub_joint_states_ = create_subscription<nao_lola_sensor_msgs::msg::JointPositions>(
      "/sensors/joint_positions", 1, [this](nao_lola_sensor_msgs::msg::JointPositions::SharedPtr sensor_joints) {
        if (pos_in_action_)
        {
  ...
  action_server_ = rclcpp_action::create_server<nao_pos_interfaces::action::PosPlay>(
      this, "nao_pos_action2",
      std::bind(&NaoPosActionServer2::handleGoal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&NaoPosActionServer2::handleCancel, this, std::placeholders::_1),
      std::bind(&NaoPosActionServer2::handleAccepted, this, std::placeholders::_1));

  RCLCPP_INFO(this->get_logger(), "NaosoccerPosActionServer initialized");
}

NaoPosActionServer2::~NaoPosActionServer2()
{
}

void NaoPosActionServer2::readPosFile(std::string& filePath)
{
...
std::string NaoPosActionServer2::getFullFilePath(std::string& filename)
{
  std::string file = "pos/" + filename;
  std::string package_share_directory = ament_index_cpp::get_package_share_directory("nao_pos_server");
...
std::vector<std::string> NaoPosActionServer2::readLines(std::ifstream& ifstream)
{
...
float NaoPosActionServer2::findElem(const std::vector<uint8_t>& indexes, const std::vector<float>& data, uint8_t joint)
{
...
void NaoPosActionServer2::calculateEffectorJoints(nao_lola_sensor_msgs::msg::JointPositions& sensor_joints)
{
  ...
  if (firstTickSinceActionStarted_)
  {
    ...
    command.positions = std::vector<float>(sensor_joints.positions.begin(), sensor_joints.positions.end());
    key_frame_start_ = std::make_unique<KeyFrame>(0, command, nao_lola_command_msgs::msg::JointStiffnesses{});
  }
  ...
  RCLCPP_DEBUG(this->get_logger(),
               ("timeFromPreviousKeyFrame, timeToNextKeyFrame, duration: " + std::to_string(timeFromPreviousKeyFrame) +
  ...
  RCLCPP_DEBUG(this->get_logger(), ("alpha, beta: " + std::to_string(alpha) + ", " + std::to_string(beta)).c_str());
...
const KeyFrame& NaoPosActionServer2::findPreviousKeyFrame(int time_ms)
{
...
const KeyFrame& NaoPosActionServer2::findNextKeyFrame(int time_ms)
{
...
bool NaoPosActionServer2::posFinished(int time_ms)
{
...
rclcpp_action::GoalResponse NaoPosActionServer2::handleGoal(
...
rclcpp_action::CancelResponse NaoPosActionServer2::handleCancel(
...
void NaoPosActionServer2::handleAccepted(
    const std::shared_ptr<rclcpp_action::ServerGoalHandle<nao_pos_interfaces::action::PosPlay>> goal_handle)
{
  std::lock_guard<std::mutex> lock(mutex_);
...
RCLCPP_COMPONENTS_REGISTER_NODE(nao_pos_action_server2_ns::NaoPosActionServer2)
